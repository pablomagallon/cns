...
function validarAltaPaciente(id_piso, id_cama) {
	...
	var diagnostico = document.getElementById('combobox').value;
	var error = "";
...
function agregarDHenCitaForma() {
	var cedula = document.getElementById('cedulaAgregar').value;
	var cedula_tipo = document.getElementById('cedulaTipoAgregar').value;
	var ap_p = document.getElementById('ap_pAgregar').value;
	var ap_m = document.getElementById('ap_mAgregar').value;
	var nombre = document.getElementById('nombreAgregar').value;
	var edad = document.getElementById('fecha_nacAgregar').value;
	var telefono = document.getElementById('telefonoAgregar').value;
	var direccion =	document.getElementById('direccionAgregar').value;
	var estado = document.getElementById('estadoAgregar').value;
	var municipio =	document.getElementById('municipioAgregar').value;
	var error = false;
	var msgerror = "";
	cedulaCorrecta = validarCedula(cedula);
	if (cedulaCorrecta != "") {
		error = true
		msgerror += "- "+cedulaCorrecta;
	}
	if (cedula_tipo.length != 2) {
		error = true
		msgerror += "- El Tipo de Cedula debe contener 2 caracteres\n\r";
	} else {
		totalTipos = tipo_cedulas.length;
		var existeTipo = false;
		for (var i = 0; i < totalTipos; i++) {
			if (tipo_cedulas[i] == cedula_tipo) existeTipo = true;
		}
		if (existeTipo == false) {
			error = true
			msgerror += "- El Tipo de Cedula ''" + cedula_tipo + "'' es incorrecto\n\r";
		}
	}
	if (ap_p.length < 1) {
		error = true
		msgerror += "- El apellido paterno es requerido\n\r";
	}
	if (ap_m.length < 1) {
		error = true
		msgerror += "- El apellido materno es requerido\n\r";
	}
	if (nombre.length < 1) {
		error = true
		msgerror += "- El nombre es requerido\n\r";
	}
	if (edad.length < 1) {
		error = true
		msgerror += "- La edad es requerida\n\r";
	}
	if (estado.length < 1) {
		error = true
		msgerror += "- El estado es requerido\n\r";
	}
	if (municipio.length < 1) {
		error = true
		msgerror += "- El municipio es requerido\n\r";
	}
	if (error) alert(msgerror); else {
		var contenedor;
		contenedor = document.getElementById('divBotones_EstadoAgregarDH');
		var objeto= new AjaxGET();
		cadena = quitarAcentos("agregarDHenCita.php?cedula=" + cedula + "&cedula_tipo=" + cedula_tipo + "&ap_p="+ap_p+"&ap_m="+ap_m+"&nombre="+nombre+"&fecha_nac="+edad+"&telefono="+telefono+"&direccion="+direccion+"&estado="+estado+"&municipio="+municipio);
		...
		objeto.onreadystatechange=function()
		{
			if (objeto.readyState==4) // Readystate 4 significa que ya acab&oacute; de cargarlo
			{
				if (objeto.responseText > 0) {
					...
					document.getElementById('cedula').value = cedula+'/'+cedula_tipo;
					document.getElementById('ap_p').value = ap_p;
					document.getElementById('ap_m').value = ap_m;
...
function validarModificarObs(id_cama, id_piso) {
...
function validarModificarObs2014(id_cama, id_piso, id_movimiento) {
	...
	if (error.length > 0) {
		...
		contenedor2 = document.getElementById('enviando');
		var objeto= new AjaxGET();
		objeto.open("GET", quitarAcentos("modificarObservaciones2014.php?id_cama="+id_cama+"&id_movimiento="+id_movimiento+"&pendientes="+pendientes+"&observaciones="+observaciones+"&interconsulta="+servicio + '_' + medico +"&diagnostico="+diagnostico),true);
		objeto.onreadystatechange=function()
		{
			if (objeto.readyState==4) // Readystate 4 significa que ya acab&oacute; de cargarlo
			{
