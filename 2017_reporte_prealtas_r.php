...
function cambiarPagina($reporteActual, $out) {
	...
	<td><table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\">
	  <tr>
		...
		<td align=\"center\">SUBDIRECCIÓN GENERAL MÉDICA<br />Reporte de Prealtas</td>
	...
	<td><table width=\"100%\" border=\"2\" cellspacing=\"0\" cellpadding=\"0\">
	  <tr>
		...
		<td width=\"50\" align=\"center\" class=\"contenido8bold\">Hrs. Prealta</td>
...
if ($_GET['tipoReporte'] == 'piso') {
	...
	for($i=0; $i<$nCamas; $i++) {
		...
		if ($paciente == "") {
			...
			$prealta = getPrealta($paciente['id_movimiento']);
			if (is_array($prealta)) {
				if ($renglon < 14)
					$renglon++;
				else
				...
				$fechaPrealta = formatoDia($prealta["fecha"], "fechaI");
				$horaPrealta = formatoHora($prealta["hora"]);
			    $horas = diferenciaHoras($fechaActual, $prealta["fecha"], $horaActual, $prealta["hora"]);
				$out .= "<tr height=\"30\">
					<td align=\"left\" class=\"contenido8\"><small>" . $piso['nombre'] . " - " . $camas[$i]['descripcion'] . "</small></td>
					<td align=\"center\" class=\"contenido8\"><small>" . $horas . " hrs.<br>" . $horaPrealta . ' ' . $fechaPrealta . "</small></td>
...
switch ($_GET["tipo"]) {
	...
	echo '
			...
			<title>REPORTE DE PREALTAS</title>
			<style type="text/css">
