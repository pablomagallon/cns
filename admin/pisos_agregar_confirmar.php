...
if ($_SESSION['idUsuario'] == "-1" ) {
	...
	if ((isset($_POST['nombre'])) && (isset($_POST['tipo']))) {
		$duplica = existeDuplica("SELECT * FROM pisos WHERE nombre='" . $_POST['nombre'] . "'");
		if(!$duplica) {
			$query = "INSERT INTO pisos VALUES(NULL,'" . $_POST['nombre'] . "','" . $_POST['tipo'] . "','')";
			$res = ejecutarSQL($query);
			if ($res[0] == 0) {// no hay error
				$out = "ok";
			} else { // hay error en el mysql
				echo "<script  language=\"javascript\" type=\"text/javascript\">alert('Error en bd al tratar de agregar el piso'); history.back();</script>";
			}
		} else {
	...
	echo "<script  language=\"javascript\" type=\"text/javascript\">alert('No existe la variable nombre del piso'); history.back();</script>";
	}
    ...
    <tr><td align="center">
    	...
    	<table class="ventana">
		  ...
		  <tr><td align="center"><br>
		      <span class="error">El piso "<b><?php echo $_POST['nombre']; ?></b>" se agregó correctamente</span><br>
		  <br>
