...
if ($_SESSION['idUsuario'] == "-1" ) {
	...
	if ((isset($_POST['nombre'])) && (isset($_POST['piso']))) {
		$duplica = existeDuplica("SELECT * FROM camas WHERE descripcion='" . $_POST['nombre'] . "' AND id_piso='" . $_POST['piso'] . "'");
		if(!$duplica) {
			$query = "INSERT INTO camas VALUES(NULL,'" . $_POST['piso'] . "','" . $_POST['nombre'] . "','1','0','')";
			$res = ejecutarSQL($query);
			if ($res[0] == 0) {// no hay error
				$out = "ok";
			} else { // hay error en el mysql
				echo "<script  language=\"javascript\" type=\"text/javascript\">alert('Error en bd al tratar de agregar la cama'); history.back();</script>";
			}
		} else {
	echo "<script  language=\"javascript\" type=\"text/javascript\">alert('Actualmente existe en la base de datos una cama llamado " . $_POST['nombre'] . ". Por favor introduzca un nombre diferente'); history.back();</script>";
		}
	} else {
	echo "<script  language=\"javascript\" type=\"text/javascript\">alert('No existe la variable nombre de la cama'); history.back();</script>";
	}
    ...
    <tr><td align="center">
    	...
    	<table class="ventana">
		  ...
		  <tr><td align="center"><br>
		      <span class="error">La cama "<b><?php echo $_POST['nombre']; ?></b>" se agregó correctamente</span><br>
		  <br>
		      <input name="continuar" id="continuar" type="button" value="Continuar" class="botones" onclick="javascript: location.replace('camas.php');" />
		      &nbsp;&nbsp;</td></tr>
        </table>
		</form>
     </td></tr></table>
