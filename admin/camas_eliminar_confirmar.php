...
if ($_SESSION['idUsuario'] == "-1" ) {
	...
	if (isset($_POST['id_cama'])) {
		$duplica = existeDuplica("SELECT * FROM camas WHERE id_cama='" . $_POST['id_cama'] . "' AND status='1'");
		if(!$duplica) {
			$query = "DELETE FROM camas WHERE id_cama='" . $_POST['id_cama'] . "' LIMIT 1";
			$res = ejecutarSQL($query);
			if ($res[0] == 0) {// no hay error
				$out = "ok";
			} else { // hay error en el mysql
				echo "<script  language=\"javascript\" type=\"text/javascript\">alert('Error en bd al tratar de eliminar el piso'); history.back();</script>";
			}
		} else {
				echo "<script  language=\"javascript\" type=\"text/javascript\">alert('La cama esta ocupada por un paciente, no se puede eliminar'); history.back();</script>";
		}
	} else {
    ...
    <tr><td align="center">
    	...
    	<table class="ventana">
		  ...
		  <tr><td align="center"><br>
		      <span class="error">La cama "<b><?php echo $_POST['nombre']; ?></b>" se elimin&oacute; correctamente</span><br>
		  <br>
