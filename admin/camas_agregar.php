...
if ($_SESSION['idUsuario'] == "-1" ) {
	echo "<script  language=\"javascript\" type=\"text/javascript\">location.replace('index.php');</script>";
		...
		$res = getPisos($_GET['id_piso']);
		$total = count($res);
		$opciones_pisos = "";
		for ($i=0; $i<$total; $i++) {
			$temp = "";
			$opciones_pisos.= "<option value=\"" . $res[$i]['id_piso'] . "\">" . $res[$i]['nombre'] . "</option>";
		}
...
<table class="tablaPrincipal" id="tablaPrincipal" width="800" height="600" cellpadding="0" cellspacing="0"><tr><td>
    ...
    <tr><td align="center">
    	<form action="camas_agregar_confirmar.php" onsubmit="MM_validateForm('nombre','','R');return document.MM_returnValue" method="post" name="forma">
    	<table class="ventana">
          <tr class="tituloVentana">
            <td colspan="2">Agregar Cama</td>
          </tr>
		  <tr>
		    <td>Nombre de la Cama:</td><td><input name="nombre" type="text" size="15" maxlength="12" id="nombre" /></td></tr>
		  <tr>
		    <td>Piso al que pertenece:</td>
		    <td><select name="piso" id="piso"><?php echo $opciones_pisos ?>
		      </select>		    </td>
		  </tr>
		  <tr><td align="center" colspan="2"><br><br>
		      <input name="cancelar" id="cancelar" type="button" value="Cancelar" class="botones" onclick="javascript: history.back();" />
		      &nbsp;&nbsp;
		       <input name="agregar" type="submit" class="botones" id="agregar" onclick="" value="Agregar Cama" />
          </td></tr>
        </table>
		</form>
     </td></tr></table>
