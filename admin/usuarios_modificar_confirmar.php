...
if ($_SESSION['idUsuario'] == "-1" ) {
	...
	if ((isset($_POST['nombre'])) && (isset($_POST['usuario']))) {
		$duplica = existeDuplica("SELECT * FROM usuarios WHERE (nombre='" . $_POST['nombre'] . "' OR login='" . $_POST['usuario'] . "') AND st='1' AND id_usuario<>'" . $_POST['id_usuario'] . "'");
		if(!$duplica) {
			$st = '';
			if ($_POST["tipo_usuario"] == "8") $st = " status='" . $_POST['piso'] . "',";
			$query = "UPDATE usuarios SET login='" . $_POST['usuario'] . "', pass='" . $_POST['contra'] . "', nombre='" . $_POST['nombre'] . "'," . $st . " tipo_usuario='" . $_POST['tipo_usuario'] . "', extra1='modificado por " . $_SESSION['idUsuario'] . "' WHERE id_usuario='" . $_POST['id_usuario'] . "' LIMIT 1";
			$res = ejecutarSQL($query);
			if ($res[0] == 0) {// no hay error
				$out = "ok";
			} else { // hay error en el mysql
				echo "<script  language=\"javascript\" type=\"text/javascript\">alert('Error en bd al tratar de modificar el usuario'); history.back();</script>";
			}
		} else {
	...
	echo "<script  language=\"javascript\" type=\"text/javascript\">alert('No existe la variable nombre o nombre de usuario'); history.back();</script>";
	}
    ...
    <tr><td align="center">
    	...
    	<table class="ventana">
		  ...
		  <tr><td align="center"><br>
		      <span class="error">El usuario "<b><?php echo $_POST['nombre']; ?></b>" se modific&oacute; correctamente</span><br>
		  <br>
