...
if ($_SESSION['idUsuario'] == "-1" ) {
	...
	if ((isset($_POST['nombre'])) && (isset($_POST['id_cama']))) {
		$duplica = existeDuplica("SELECT * FROM camas WHERE descripcion='" . $_POST['nombre'] . "' AND id_cama<>'" . $_POST['id_cama'] . "'");
		if(!$duplica) {
			$query = "UPDATE camas SET descripcion='" . $_POST['nombre'] . "', id_piso='" . $_POST['piso'] . "' WHERE id_cama='" . $_POST['id_cama'] . "' LIMIT 1";
			$res = ejecutarSQL($query);
			if ($res[0] == 0) {// no hay error
				$out = "ok";
			} else { // hay error en el mysql
				echo "<script  language=\"javascript\" type=\"text/javascript\">alert('Error en bd al tratar de modificar la cama'); history.back();</script>";
			}
		} else {
	echo "<script  language=\"javascript\" type=\"text/javascript\">alert('Actualmente existe en la base de datos una cama llamada " . $_POST['nombre'] . ". Por favor introduzca un nombre diferente'); history.back();</script>";
		}
	} else {
    ...
    <tr><td align="center">
    	...
    	<table class="ventana">
          <tr class="tituloVentana">
            <td>Modificar Cama</td>
          </tr>
		  <tr><td align="center"><br>
		      <span class="error">La cama "<b><?php echo $_POST['nombre']; ?></b>" se modific&oacute; correctamente</span><br>
		  <br>
